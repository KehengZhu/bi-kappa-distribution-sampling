...
def g6_evidence(ctx: Context, p1: dict, override_path: str | None) -> dict:
    ...
    # Two G6 facts are about this script's own output and so cannot be observed from inside
    # it: whether `make verify` passes over manifests that hash the results this run is
    # writing, and whether `make reverify` regenerates them byte for byte.  Both are read
    # from a receipt the release step writes after measuring them.  Absent, they stay None
    # and G6 fails naming them, which is the honest state of a run that has not been
    # re-verified yet.
    #
    # A receipt is a measurement carried across a process boundary, and it is only that if
    # it belongs to THIS run.  The third holdout found out what happens when nothing checks:
    # `results/g6_evidence.json` survived from the second holdout -- results/ is rewritten
    # per run but the receipt is an input, not an output -- and carried `make verify` exit 0,
    # `make reverify` true and the archive identifier of commit e5c9837 into a run where
    # none of the three had been done.  G6 passed on another run's evidence.  Two rules
    # follow, and between them a stale or optimistic receipt cannot carry this gate:
    #
    #   1. The receipt must NAME this run -- the commit the data were produced at and the
    #      SHA-256 of the protocol they were produced under.  A receipt that names neither,
    #      or names something else, is rejected whole, and the rejection is published in
    #      `g6_receipt` rather than passed over in silence.
    #   2. It may supply only what this script cannot measure.  `dependencies_clean` and
    #      `baseline_comparison_resolved` are measured above and are never taken from a
    #      receipt.  `make_verify_exit_code` is measured above too whenever the manifests
    #      exist, and a live measurement always wins: a receipt may fill the gap when there
    #      is no manifest to read, but it may never overwrite a manifest that was read and
    #      failed.
    ev["make_reverify_identical"] = None
    measured_verify = ev["make_verify_exit_code"]
    run_commit = None
    if os.path.exists(envjson):
    ...
    receipt_commit = supplied.get("run_commit")
    receipt_proto = supplied.get("protocol_sha256")
    if receipt_commit != run_commit or receipt_proto != ctx.proto.sha256:
        ev["g6_receipt"] = (
            f"REJECTED: the receipt names run_commit={receipt_commit!r} under protocol "
            f"{receipt_proto!r}, and this run is {run_commit!r} under {ctx.proto.sha256!r}. "
            "A receipt from another run is not evidence about this one.")
        return ev

    ev["g6_receipt"] = f"accepted: measured at {run_commit} under protocol {receipt_proto}"
    for k in ("make_reverify_identical", "archive_identifier"):
        if k in supplied:
            ev[k] = supplied[k]
    if "make_verify_exit_code" in supplied:
        if measured_verify is None:
            ev["make_verify_exit_code"] = supplied["make_verify_exit_code"]
        elif int(supplied["make_verify_exit_code"]) != int(measured_verify):
            ev["g6_receipt"] += (
                f"; its make_verify_exit_code={supplied['make_verify_exit_code']} is "
                f"IGNORED -- the manifests were read here and gave {measured_verify}")
    return ev
